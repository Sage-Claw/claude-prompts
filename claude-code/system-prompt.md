...
[betas] Stripped from tool schemas: [{{...}}] (CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1)
...
For broader codebase exploration and deep research, use the {{...}} tool with subagent_type={{...}}. This is slower than using {{...}} directly, so use this only when a simple, directed search proves to be insufficient or when your task will clearly require more than {{...}} queries.
...
If a tool named above appears in the deferred-tool list for this session, load it with {{...}} (`select:<tool_name>`) before calling it. The deferred-tool list is in the conversation; a tool that appears only there has no schema loaded yet and will fail if called directly.
